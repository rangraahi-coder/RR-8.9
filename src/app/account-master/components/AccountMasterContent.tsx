...
export default function AccountMasterContent({ lang = 'en' }: AccountMasterContentProps) {
  ...
  const loadAccounts = useCallback(async () => {
    if (!accountsLoaded.current) setLoading(true);
    try {
  ...
  const handleSave = async (updated: Account) => {
    ...
    if (result.data) {
      ...
      return; // Keep the edited draft open when saving fails.
    }
    setEditAccount(null);
    refreshAccounts();
  };
  ...
  const handleAddNew = async (newAccount: Account) => {
    ...
    if (result.data) {
      ...
      return; // Keep all typed fields available for correction/retry.
    }
