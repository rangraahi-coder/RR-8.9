...
export default function DashboardContent({lang='en'}:{lang?:'en'|'hi'}){
 ...
 const load=useCallback(async()=>{
  ...
  const definitions:{module:string;key:string;title:string;table:string;select:string;unit:string;sum?:string;filter?:(r:any)=>boolean;href:string}[]=[
   ...
   {"module": "cutting", "key": "cut_fabric", "title": "Cutting fabric consumed", "table": "cutting_entries", "select": "*", "unit": "metres", "href": "/cutting", "sum": "fabric_consumed_qty",filter:r=>r.status!=='cancelled'&&['metres','meters','m','mtr'].includes(String(r.unit).toLowerCase())},
   ...
   {module:'cutting',key:'cut_average',title:'Cutting table average',table:'cutting_entries',select:'*',unit:'metres / cut piece',href:'/cutting',filter:r=>r.status!=='cancelled'&&['metres','meters','m','mtr'].includes(String(r.unit).toLowerCase())},
  ];
 ...
 <section className="card-surface p-4"><h2 className="font-semibold mb-3">Recent work</h2>{Array.from(new Map(metrics.flatMap(m=>(m.rows||[]).filter(r=>r.created_at&&(r.voucher_no||r.entry_no||r.receipt_no||r.issue_no||r.order_no)).map(r=>[m.href+':'+r.id,{r,href:m.href} as const] as const))).values()).sort((a,b)=>String(b.r.created_at).localeCompare(String(a.r.created_at))).slice(0,8).map(({r,href})=><Link key={href+r.id} href={href} className="block border-t py-3"><span>{r.voucher_no||r.entry_no||r.receipt_no||r.issue_no||r.order_no}</span><small className="block text-muted-foreground">{r.created_by||'User not recorded'} · {new Date(r.created_at).toLocaleString('en-IN',{timeZone:'Asia/Kolkata'})} IST</small></Link>)}</section>
 <details open className="card-surface p-5"><summary className="cursor-pointer font-700">Department totals</summary><div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4">{metrics.filter(m=>!['pieces',...featured.map(f=>f.key)].includes(m.key)).map(m=><button key={m.key} onClick={()=>setBreakdown(m)} className="border rounded-xl p-4 text-left hover:bg-muted"><p className="text-sm">{lang==='hi'?(labels[m.key]||m.title):m.title}</p><p className="font-700 text-xl">{loading||m.error?'—':m.value.toLocaleString('en-IN')}</p><p className="text-xs text-muted-foreground">{m.unit}</p></button>)}</div></details>
