 ...
 const load=useCallback(async()=>{
  ...
  const result=await Promise.all(definitions.filter(d=>can(d.module)).map(async d=>{
   ...
   try{
    ...
    const included=d.filter?rows.filter(d.filter):rows;metric.rows=included;metric.sum=d.sum;metric.value=d.sum?included.reduce((sum,r)=>sum+Number(r[d.sum!]||0),0):included.length;
