...
import {TEAM_PROFILES,staffEmail} from '@/lib/teamProfiles';
export async function POST(request:NextRequest){
 try{
  ...
  if(body.operation==='reset-password'){
   ...
   const {error:resetError}=await admin.auth.admin.updateUserById(body.userId,{password:body.password,user_metadata:{...target.user.user_metadata,must_change_password:true}});
   if(resetError)return NextResponse.json({error:resetError.message},{status:400});
   return NextResponse.json({ok:true});
  }
  const preset=body.username?TEAM_PROFILES.find(p=>p.username===body.username):null;
  if(body.username&&!preset)return NextResponse.json({error:'Unknown team username'},{status:400});
  const permissions=preset?.permissions||body.permissions||{};
  ...
  const admin=createClient(url,secret,{auth:{persistSession:false,autoRefreshToken:false}});
  const {data,error:createError}=await admin.auth.admin.createUser({email:email.trim(),password:body.password,email_confirm:true,user_metadata:{full_name:preset?.name||String(body.name||''),must_change_password:true}});
  if(createError||!data.user)return NextResponse.json({error:createError?.message||'User could not be created. Existing passwords were not changed.'},{status:400});
  const {error:accessError}=preset?await caller.rpc('erp_apply_team_profile',{p_user_id:data.user.id,p_username:preset.username}):await caller.rpc('erp_set_user_access',{p_user_id:data.user.id,p_name:String(body.name||''),p_active:body.active===true,p_admin:false,p_permissions:permissions});
  if(accessError)return NextResponse.json({error:'Login created without ERP access. Select the account and assign permissions. '+accessError.message,userId:data.user.id},{status:409});
  return NextResponse.json({ok:true});
