...
function rowToCuttingEntry(row: any, subComponents: any[]): CuttingEntry {
  return {
    ...
    subComponentDetails: subComponents.map((sc: any): SubComponentCutDetail => ({
      ...
      component: sc.component, cuttingRateBasis: sc.cutting_rate_basis || 'piece', cuttingChargeQty: sc.cutting_charge_qty == null ? undefined : Number(sc.cutting_charge_qty), cuttingRate: sc.cutting_rate == null ? undefined : Number(sc.cutting_rate), stitchingRate: sc.stitching_rate == null ? undefined : Number(sc.stitching_rate),
...
export const cuttingService = {
  ...
  async create(
    ...
    const {data:savedEntry,error}=await supabase.rpc('erp_save_team_voucher',{p_kind:'cutting',p_id:null,p_header:row,p_lines:entry.subComponentDetails.map(sc=>({component:sc.component,cutting_rate:sc.cuttingRate??null,cutting_rate_basis:sc.cuttingRateBasis||'piece',cutting_charge_qty:sc.cuttingChargeQty??sc.netPieces,stitching_rate:sc.stitchingRate??null,total_pieces:sc.totalPieces,rejections:sc.rejections,net_pieces:sc.netPieces,size_breakdown:sc.sizes}))});
    ...
    return rowToCuttingEntry(savedEntry, entry.subComponentDetails.map((sc) => ({
      component: sc.component, cutting_rate: sc.cuttingRate, cutting_rate_basis: sc.cuttingRateBasis||'piece', cutting_charge_qty: sc.cuttingChargeQty??sc.netPieces, stitching_rate: sc.stitchingRate,
  ...
  async update(
    ...
    const {data:savedEntry,error}=await supabase.rpc('erp_save_team_voucher',{p_kind:'cutting',p_id:id,p_header:row,p_lines:entry.subComponentDetails.map(sc=>({component:sc.component,cutting_rate:sc.cuttingRate??null,cutting_rate_basis:sc.cuttingRateBasis||'piece',cutting_charge_qty:sc.cuttingChargeQty??sc.netPieces,stitching_rate:sc.stitchingRate??null,total_pieces:sc.totalPieces,rejections:sc.rejections,net_pieces:sc.netPieces,size_breakdown:sc.sizes}))});
    if(error)throw new Error(error.message);
    if(typeof window!=='undefined')window.dispatchEvent(new Event('erp-data-changed'));
    return rowToCuttingEntry(savedEntry, entry.subComponentDetails.map((sc) => ({
      component: sc.component, cutting_rate: sc.cuttingRate, cutting_rate_basis: sc.cuttingRateBasis||'piece', cutting_charge_qty: sc.cuttingChargeQty??sc.netPieces, stitching_rate: sc.stitchingRate,
