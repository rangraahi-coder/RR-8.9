...
function Content({render,pageTitle,pageTitleHi}:Props){
 ...
 else if(!loading&&!accessLoading&&sessionStatus==='signed-in'&&!canAccessRoute(pathname))router.replace(getDefaultRoute());},[loading,accessLoading,sessionStatus,pathname,router,canAccessRoute,getDefaultRoute]);
