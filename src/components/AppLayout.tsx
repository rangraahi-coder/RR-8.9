...
function Content({render,pageTitle,pageTitleHi}:Props){
 ...
 return <div className="flex h-screen overflow-hidden bg-background"><Sidebar collapsed={collapsed} onToggle={()=>setCollapsed(v=>!v)}/><div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${collapsed?'ml-16':'ml-64'}`}><TopBar onSearch={setSearch}/><main className="flex-1 overflow-y-auto scrollbar-thin"><div className="px-6 py-6 xl:px-8 2xl:px-10 max-w-screen-2xl mx-auto"><div className="mb-5 text-sm text-muted-foreground">Rangraahi Powerhouse <span className="mx-2">›</span> {lang==='hi'?pageTitleHi:pageTitle}</div><a href="/change-password" className="float-right text-xs text-muted-foreground underline">Change password</a><VoucherHistory/>{render(lang,search)}</div></main></div></div>;
}
