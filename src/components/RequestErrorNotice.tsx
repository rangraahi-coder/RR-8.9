...
export default function RequestErrorNotice(){
 ...
 useEffect(()=>{const onError=(event:Event)=>{const issue=(event as CustomEvent<NavigableIssue>).detail;setMessages(old=>old.some(x=>x.message===issue.message&&x.target===issue.target)?old:[...old,issue].slice(0,10));};window.addEventListener('erp-request-error',onError);return()=>window.removeEventListener('erp-request-error',onError);},[]);
