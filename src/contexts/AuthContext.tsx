...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [verifiedUser, setVerifiedUserState] = useState<any>(null);
  const verifiedIdentity = useRef<string | null>(null);
  const setVerifiedUser = useCallback((next: any) => {
    verifiedIdentity.current = next?.id || null;
    setVerifiedUserState(next);
  }, []);
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (newSession?.user) {
        ...
        // Same-user token/focus verification must not unmount the workspace.
        if (verifiedIdentity.current !== newSession.user.id) {
          setVerifiedUser(null);
  ...
  const refreshAccess=useCallback(async()=>{
    ...
    try{const {data,error}=await supabase.from('erp_user_access').select('*').eq('user_id',verifiedUser.id).maybeSingle();
      ...
      if(error)throw error;setAccessProfile(previous=>JSON.stringify(previous)===JSON.stringify(data)?previous:data);setAccessError('');
